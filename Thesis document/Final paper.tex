...
{\bf Izvle\v cek:}\\
Elektroencefalografski (EEG) posnetki so ključni za analizo možganske aktivnosti, vendar so pogosto moteni zaradi artefaktov, med katerimi so najbolj značilni artefakti, ki so posledica mežikanja. Tradicionalne metode odstranjevanja artefaktov, kot je z analizo neodvisnih komponent (ICA), so učinkovite, a računsko zahtevne in neprimerne za uporabo v realnem času. Cilj diplomske naloge je z uporabo globokega učenja razviti model za zaznavo mežikanj v EEG posnetkih z uporabo časovnih konvolucijskih nevronskih mrež (ang. Temporal Convolutional Network - TCN). V raziskavi je bil uporabljen nabor EEG podatkov, prvotno zbran za raziskovanje učinkov kofeina. Analiza se osredotoča zgolj na zaznavo artefaktov. Podatki so bili predhodno obdelani, ICA pa je služila kot osnova za označevanje učnih podatkov. Predlagani TCN model sestoji iz zaporedja več konvolucijskih blokov na različnih stopnjah podvzorčenja signalov, za upoštevanje dolgoročnih časovnih odvisnosti. Rezultati kažejo visoko točnost (98,28\%) in več kot 90\% uspešnost zaznave artefaktov, primerljivo z ICA. Model sicer prekomerno zaznava artefakte, kar vodi do zmernega števila napačnih pozitivnih zaznav. Primerjava z enostavnejšimi TCN arhitekturami potrjuje prednosti večje kompleksnosti z vidika točnosti. Študija dokazuje, da lahko modeli, kot je TCN, učinkovito nadomestijo ICA pri zaznavanju artefaktov v EEG posnetkih. Pristop omogoča avtomatizirano, neodvisno od posameznikov in razširljivo zaznavo motenj, kar je pomembno za nadaljnje raziskave v nevroznanosti in vmesnikih človek–računalnik.
\vspace{0.2cm}
}}
\end{center}
